...
///
/// Classes for keeping track of spaces and registers (for a single
/// architecture).
...
///
/// This can turn up in various XML configuration files and essentially acts as
/// a command to override the size of an address space as defined by the
/// architecture
class TruncationTag {
    string spaceName; ///< Name of space to be truncated
    uint4 size; ///< Size truncated addresses into the space
public:
    // Restore \b this from XML
...
public:
    ...
    static const uint1* unpackVarnodeData (const uint1* ptr, VarnodeData& v,
                                           const AddrSpaceManager* trans);
    /// Emit pcode directly from a packed byte stream
    const uint1* restorePackedOp (const Address& addr, const uint1* ptr,
                                  const AddrSpaceManager* trans);
};
...
///
/// This class is used if there is a special calculation to get from a constant
/// embedded in the code being analyzed to the actual Address being referred to.
/// This is used especially in the case of a segmented architecture, where
/// "near" pointers must be extended to a full address with implied segment
/// information.
...
public:
    ...
    ///
    /// Given a native constant in a specific context, resolve what address is
    /// being referred to.
    ...
    /// \param fullEncoding is used to hold the full pointer encoding if \b val is
    ///        a partial encoding
...
class SpacebaseSpace : public AddrSpace {
    ...
    void setBaseRegister (
        ...
        bool stackGrowth); ///< Set the base register at time space is created
public:
    SpacebaseSpace (AddrSpaceManager* m, const Translate* t, const string& nm,
                    int4 ind, int4 sz, AddrSpace* base, int4 dl);
...
/// multiple physical memory locations. This record describes such a split. The
/// pieces must be listed from \e most \e significant to \e least \e
/// significant.
class JoinRecord {
    friend class AddrSpaceManager;
...
protected:
    ...
    void addSpacebasePointer (SpacebaseSpace* basespace,
...
public:
    ...
    Address resolveConstant (AddrSpace* spc, uintb val, int4 sz,
                             const Address& point, uintb& fullEncoding) const;
    ...
    JoinRecord* findAddJoin (const vector<VarnodeData>& pieces,
                             uint4 logicalsize);
    ...
    /// \brief Build a logically lower precision storage location for a bigger
    /// floating point register
    Address constructFloatExtensionAddress (const Address& realaddr,
                                            int4 realsize, int4 logicalsize);
    /// \brief Build a logical whole from register pairs
    Address constructJoinAddress (const Translate* translate,
...
public:
    ...
    virtual void addRegister (const string& nm, AddrSpace* base, uintb offset,
                              int4 size) = 0;
    ...
    virtual string getRegisterName (AddrSpace* base, uintb off,
                                    int4 size) const = 0;

    /// \brief Get a list of all register names and the corresponding location
    ///
    /// Most processors have a list of named registers and possibly other memory
    /// locations that are specific to it. This function populates a map from
    /// the location information to the name, for every named location known by
    /// the translator \param reglist is the map which will be populated by the
    /// call
    ...
    virtual int4 oneInstruction (PcodeEmit& emit,
                                 const Address& baseaddr) const = 0;
    ...
    virtual int4 printAssembly (AssemblyEmit& emit,
                                const Address& baseaddr) const = 0;
};
...
/// Although endianness is usually specified on the space, most languages set an
/// endianness across the entire processor. This routine sets the endianness to
/// \b big if the -val- is passed in as \b true. Otherwise, the endianness is
/// set to \b small. \param val is \b true if the endianness should be set to \b
/// big
