...
AddrSpace* SleighBuilder::generatePointer (const VarnodeTpl* vntpl,
                                           VarnodeData& vn)

{ // Generate the pointer varnode -vn- from a dynamic template -vntpl-
    const FixedHandle& hand (
        walker->getFixedHandle (vntpl->getOffset ().getHandleIndex ()));
...
{ // Dump on op through low-level dump interface
    ...
    if (outvn != (VarnodeTpl*)0) {
        if (outvn->isDynamic (*walker)) {
            ...
            generateLocation (
                outvn,
                storevars[2]); // Output of -op- is really temporary storage
...
// BUILD directives
{
    ...
    for (int4 i = 0; i < numops; ++i) {
        SubtableSymbol* sym =
            (SubtableSymbol*)ct->getOperand (i)->getDefiningSymbol ();
        ...
        ConstructTpl* construct =
            walker->getConstructor ()->getNamedTempl (secnum);
...
SleighBuilder::SleighBuilder (ParserWalker* w, DisassemblyCache* dcache,
                              PcodeCacher* pc, AddrSpace* cspc, AddrSpace* uspc,
                              uint4 umask) :
PcodeBuilder (0)
{
    walker = w;
...
// Append pcode for a particular build statement
{
    ...
    int4 index = bld->getIn (0)->getOffset ().getReal ();
    // Check if operand is a subtable
    SubtableSymbol* sym = (SubtableSymbol*)walker->getConstructor ()
                              ->getOperand (index)
                              ->getDefiningSymbol ();
    if ((sym == (SubtableSymbol*)0) ||
        (sym->getType () != SleighSymbol::subtable_symbol))
        return;
...
{ // Append pcode for an entire instruction (delay slot)
    ...
    do {
        ...
        walker = &newwalker;
        walker->baseState ();
        build (walker->getConstructor ()->getTempl (),
               -1); // Build the whole delay slot
        fallOffset += len;
        bytecount += len;
...
DisassemblyCache::DisassemblyCache (ContextCache* ccache, AddrSpace* cspace,
                                    int4 cachesize, int4 windowsize)
...
{ // Return a (possibly cached) ParserContext that is associated with -addr-
    ...
    // If n different calls to this interface are made with n different
    // Addresses, if
    //    n <= minimumreuse   AND
    //    all the addresses are within the windowsize (=mask+1)
    // then the cacher guarantees that you get all different ParserContext
    // objects
    ...
    if (nextfree >= minimumreuse)
        nextfree = 0;
    res->setAddr (addr);
    // Need to start over with parsing
    res->setParserState (ParserContext::uninitialized);
    // Stick it into the hashtable
    hashtable[hashindex] = res;
    return res;
}

Sleigh::Sleigh (LoadImage* ld, ContextDatabase* c_db) : SleighBase ()
...

{
    ...
    discache = new DisassemblyCache (cache, getConstantSpace (),
                                     parser_cachesize, parser_windowsize);
}

ParserContext* Sleigh::obtainContext (const Address& addr, int4 state) const
// Obtain a ParserContext for the instruction at the given -addr-. This may be
// cached.
{
...
// Resolve ALL the constructors involved in the
{
    ...
    while (walker.isState ()) {
        ...
        while (oper < numoper) {
            ...
            off = walker.getOffset (sym->getOffsetBase ()) +
                  sym->getRelativeOffset ();
    ...
    while (walker.isState ()) {
        ...
        while (oper < numoper) {
            ...
            if (triple != (TripleSymbol*)0) {
                ...
                // Result of expression is a constant
...
void Sleigh::getInstructionBytes (const Address& baseaddr, uint1* buf) const

{
    ...
    do {
        ...
        memcpy(&buf[bytestart], (uint1*)&tmp, sizeof(uint1));
