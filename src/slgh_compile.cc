...
void WithBlock::set (SubtableSymbol* s, PatternEquation* pq,
...

{
    ...
    if (cvec != (vector<ContextChange*>*)0) {
        for (int4 i = 0; i < cvec->size (); ++i)
            contvec.push_back (
                (*cvec)[i]); // Lay claim to -cvec-s pointers, we don't clone
        delete cvec;
    }
}
...
PatternEquation*
WithBlock::collectAndPrependPattern (const list<WithBlock>& stack,
...
vector<ContextChange*>*
WithBlock::collectAndPrependContext (const list<WithBlock>& stack,
...
ConsistencyChecker::ConsistencyChecker (SubtableSymbol* rt, bool un,
                                        bool warndead)
...
int4 ConsistencyChecker::recoverSize (const ConstTpl& sizeconst,
                                      Constructor* ct)

{
    ...
    case ConstTpl::handle:
        ...
        if (size == -1) {
            tabsym =
                dynamic_cast<SubtableSymbol*> (opsym->getDefiningSymbol ());
...
{ // Deal with detected extension (SEXT or ZEXT) where the
    ...
    if (printextwarning) {
        cerr << "Unnecessary ";
        printOpName (cerr, op);
        cerr << " in constructor starting at line " << dec << ct->getLineno ()
             << endl;
    }
...

{
    ...
    case CPUI_INT_LESS: {
        ...
        if (vn->getSpace ().isConstSpace () && vn->getOffset ().isZero ()) {
            cerr
                << "Unsigned comparison with zero is always false in constructor starting at line "
                << dec << ct->getLineno () << endl;
        }
    } break;
    default:
        break;
...
{ // Make sure op template meets size restrictions
    ...
    case CPUI_FLOAT_ROUND:
        ...
        if (vnout == -1) {
            printOpError (op, ct, -1, -1,
                          "Using subtable with exports in expression");
            return false;
        }
    ...
    case CPUI_FLOAT_SUB:
        ...
        if ((vnout != 0) && (vn0 != 0) && (vnout != vn0)) {
            printOpError (op, ct, -1, 0,
                          "The output and all input sizes must match");
            return false;
        }
    ...
    case CPUI_INT_SRIGHT:
        ...
        if (vnout != vn0) {
            printOpError (op, ct, -1, 0,
                          "Output and first input must be the same size");
            return false;
        }
        return true;
    case CPUI_INT_ZEXT:
    ...
    case CPUI_STORE:
        ...
        if ((vn1 != 0) && (vn1 != spc->getAddrSize ())) {
            printOpError (op, ct, 1, 1,
                          "Pointer size must match size of space");
            return false;
        }
        return true;
    case CPUI_SUBPIECE:
...
OperandSymbol* ConsistencyChecker::getOperandSymbol (int4 slot, OpTpl* op,
                                                     Constructor* ct)
...
void ConsistencyChecker::printOpError (OpTpl* op, Constructor* ct, int4 err1,
                                       int4 err2, const string& msg)

{
    ...
    if (err2 != err1)
        ...
        op2 = (OperandSymbol*)0;
    cerr << "Size restriction error in table \"" << sym->getName () << "\""
         << endl;
    cerr << "  in constructor starting at line " << dec << ct->getLineno ()
         << endl;
...
bool ConsistencyChecker::checkConstructorSection (Constructor* ct,
                                                  ConstructTpl* cttpl)
...
bool ConsistencyChecker::checkSectionTruncations (Constructor* ct,
                                                  ConstructTpl* cttpl,
                                                  bool isbigendian)
...

{
    ...
    for (int4 i = 0; i < numconstruct; ++i) {
        ...
        if (exportres != (HandleTpl*)0) {
            if (seenemptyexport && (!seennonemptyexport)) {
                cerr << "Table " << sym->getName () << " exports inconsistently"
                     << endl;
                cerr << "Constructor starting at line " << dec
                     << ct->getLineno () << " is first inconsistency" << endl;
                testresult = false;
            }
            ...
            if ((exsize != 0) && (exsize != tablesize)) {
                cerr << "Table " << sym->getName ()
                     << " has inconsistent export size." << endl;
                cerr << "Constructor starting at line " << dec
                     << ct->getLineno () << " is first conflict" << endl;
                testresult = false;
            }
        } else {
    ...
    if (seennonemptyexport) {
        if (tablesize == 0)
            cerr << "Warning: Table " << sym->getName () << " exports size 0"
                 << endl;
...
bool ConsistencyChecker::possibleIntersection (const VarnodeTpl* vn1,
                                               const VarnodeTpl* vn2)
...
bool ConsistencyChecker::readWriteInterference (const VarnodeTpl* vn,
                                                const OpTpl* op,
...
void ConsistencyChecker::examineVn (map<uintb, OptimizeRecord>& recs,
                                    const VarnodeTpl* vn, uint4 i, int4 inslot,
                                    int4 secnum)
{ // If varnode is a temporary,  count whether it is read or written
    ...
    iter = recs.insert (pair<uint4, OptimizeRecord> (
                            vn->getOffset ().getReal (), OptimizeRecord ()))
               .first;
    if (inslot >= 0) {
...
void ConsistencyChecker::optimizeGather1 (Constructor* ct,
...
{ // Make sure any temp used by the export is not optimized away
    ...
    if (hand->getPtrSpace ().isUniqueSpace ()) {
        if (hand->getPtrOffset ().getType () == ConstTpl::real) {
            ...
            res = recs.insert (
                pair<uintb, OptimizeRecord> (offset, OptimizeRecord ()));
...
ConsistencyChecker::OptimizeRecord*
ConsistencyChecker::findValidRule (Constructor* ct,
                                   map<uintb, OptimizeRecord>& recs) const
    ...
    while (iter != recs.end ()) {
        ...
        if ((currec.writecount == 1) && (currec.readcount == 1) &&
            ...
            if (op->getOpcode () == CPUI_COPY) {
                ...
                const VarnodeTpl* vn = op->getOut ();
                for (int4 i = currec.writeop + 1; i < currec.readop;
                     ++i) { // Check for interference between write and read
...
void ConsistencyChecker::applyOptimization (Constructor* ct,
...
void ConsistencyChecker::checkUnusedTemps (
    Constructor* ct, const map<uintb, OptimizeRecord>& recs)

{
    ...
    while (iter != recs.end ()) {
        ...
        if (currec.readcount == 0) {
            if (printdeadwarning)
                cerr
                    << "Warning: temporary is written but not read in constructor starting at line "
                    << dec << ct->getLineno () << endl;
            writenoread += 1;
        } else if (currec.writecount == 0) {
            cerr
                << "Error: temporary is read but not written in constructor starting at line "
                << dec << ct->getLineno () << endl;
            readnowrite += 1;
        }
        ++iter;
...
{ // Fix handle details of a macro generated OpTpl relative to its specific invocation
    ...
    for (int4 i = 0; i < op->numInput (); ++i) {
        ...
        if (plus >= 0) {
            ...
            uintb newtemp =
                slgh->getUniqueAddr (); // Generate a new temporary location

            // Generate a SUBPIECE op that implements the offset_plus
            OpTpl* subpieceop = new OpTpl (CPUI_SUBPIECE);
            VarnodeTpl* newvn =
                new VarnodeTpl (ConstTpl (slgh->getUniqueSpace ()),
                                ConstTpl (ConstTpl::real, newtemp),
                                ConstTpl (ConstTpl::real, realsize));
            subpieceop->setOutput (newvn);
            HandleTpl* hand = params[handleIndex];
            VarnodeTpl* origvn = new VarnodeTpl (
                hand->getSpace (), hand->getPtrOffset (), hand->getSize ());
            subpieceop->addInput (origvn);
            VarnodeTpl* plusvn =
                new VarnodeTpl (ConstTpl (slgh->getConstantSpace ()),
                                ConstTpl (ConstTpl::real, plus),
                                ConstTpl (ConstTpl::real, 4));
            subpieceop->addInput (plusvn);
            outvec.push_back (subpieceop);

            delete vn; // Replace original varnode
            op->setInput (new VarnodeTpl (*newvn),
                          i); // with output of subpiece
        }
    }
    outvec.push_back (op);
    return true;
...
SleighCompile::SleighCompile (void) : SleighBase ()
{
...
{ // Define the "pre" defined spaces and symbols
    ...
    insertSpace (new ConstantSpace (this, this, "const",
                                    AddrSpace::constant_space_index));
    SpaceSymbol* spacesym =
        new SpaceSymbol (getConstantSpace ()); // Constant space
    symtab.addSymbol (spacesym);
    OtherSpace* otherSpace =
        new OtherSpace (this, this, "OTHER", AddrSpace::other_space_index);
...

{
    ...
    if ((sym->getSize ()) % 4 != 0)
        reportError ("Invalid size of context register: " + sym->getName () +
    ...
    while (i < sz) {
        ...
        if ((max - min) > (8 * sizeof (uintm)))
            reportError ("Size of bitfield " + qual->name +
                             " larger than 32-bits",
                         false);
        if (max > maxbits)
            reportError ("Scope of bitfield " + qual->name +
                             " extends beyond the size of context register",
                         false);
        j = i + 1;
        ...
        for (; i < j; ++i) {
            ...
            addSymbol (new ContextSymbol (qual->name, field, sym, qual->low,
                                          qual->high, qual->flow));
        }
    }
    sym->markAsContext ();
...

{
    ...
    for (int4 i = 0; i < tables.size (); ++i) {
        ...
        if (tables[i]->getPattern () == (TokenPattern*)0)
            reportWarning ("Unreferenced table: " + tables[i]->getName (),
                           false);
    }
}
...

{
    ...
    if ((!warnunnecessarypcode) && (checker.getNumUnnecessaryPcode () > 0)) {
        ...
        cerr << " unnecessary extensions/truncations were converted to copies"
             << endl;
...
int4 SleighCompile::findCollision (map<uintb, int4>& local2Operand,
                                   const vector<uintb>& locals, int operand)
    ...
    for (int4 i = 0; i < ct->getNumOperands (); ++i) {
        ...
        if (collideOperand >= 0) {
            ...
            if (warnalllocalcollisions) {
                ...
                cerr << " in constructor starting at line " << dec
                     << ct->getLineno () << endl;
            }
...

{
    ...
    if (collisionCount > 0) {
        cerr << "WARNING: " << dec << collisionCount
             << " constructors with local collisions between operands" << endl;
        if (!warnalllocalcollisions)
...

{
    if (noplist.size () > 0) {
        ...
        cerr << dec << (int4)noplist.size () << " NOP constructors found"
             << endl;
        if (!warnallnops)
...
{ // Make sure label symbols are used properly
    ...
    for (iter = scope->begin (); iter != scope->end (); ++iter) {
        ...
        if (sym->getRefCount () == 0)
            s << "   Label <" << sym->getName ()
              << "> was placed but not used\n";
        else if (!sym->isPlaced ())
            s << "   Label <" << sym->getName ()
              << "> was referenced but never placed\n";
    }
    return s.str ();
}
...

{
    ...
    if ((size & 7) != 0) {
        ...
        size = size / 8;
    Token* newtoken =
        new Token (*name, size, isBigEndian (), tokentable.size ());
...
{
    TokenField* field =
        new TokenField (sym->getToken (), qual->signext, qual->low, qual->high);
...

{
    ...
    AddrSpace* spc = new AddrSpace (this, this, IPTR_PROCESSOR, qual->name,
                                    qual->size, qual->wordsize, numSpaces (),
    ...
    if (qual->isdefault) {
        if (getDefaultSpace () != (AddrSpace*)0)
            ...
            setDefaultSpace (
                spc->getIndex ()); // Make the flagged space the default
...
void SleighCompile::defineVarnodes (SpaceSymbol* spacesym, uintb* off,
                                    uintb* size, vector<string>* names)
...
void SleighCompile::defineBitrange (string* name, VarnodeSymbol* sym,
                                    uint4 bitoffset, uint4 numb)
...
void SleighCompile::attachValues (vector<SleighSymbol*>* symlist,
                                  vector<intb>* numlist)

{
    SleighSymbol* dupsym = dedupSymbolList (symlist);
    if (dupsym != (SleighSymbol*)0)
        reportWarning ("\"attach values\" list contains duplicate entries: " +
                           dupsym->getName (),
                       true);
    for (int4 i = 0; i < symlist->size (); ++i) {
        ...
        if (patval->maxValue () + 1 != numlist->size ()) {
            reportError ("Attach value " + sym->getName () +
                             " is wrong size for list",
                         true);
        }
        symtab.replaceSymbol (sym, new ValueMapSymbol (sym->getName (), patval,
                                                       *numlist));
    }
    delete numlist;
    delete symlist;
...
void SleighCompile::attachNames (vector<SleighSymbol*>* symlist,
                                 vector<string>* names)

{
    SleighSymbol* dupsym = dedupSymbolList (symlist);
    if (dupsym != (SleighSymbol*)0)
        reportWarning ("\"attach names\" list contains duplicate entries: " +
                           dupsym->getName (),
                       true);
    for (int4 i = 0; i < symlist->size (); ++i) {
        ValueSymbol* sym = (ValueSymbol*)(*symlist)[i];
        ...
        if (patval->maxValue () + 1 != names->size ()) {
            reportError ("Attach name " + sym->getName () +
                             " is wrong size for list",
                         true);
        }
        symtab.replaceSymbol (sym,
                              new NameSymbol (sym->getName (), patval, *names));
    }
    delete names;
    delete symlist;
}

void SleighCompile::attachVarnodes (vector<SleighSymbol*>* symlist,
...

{
    ...
    for (int4 i = 0; i < symlist->size (); ++i) {
        ...
        if (patval->maxValue () + 1 != varlist->size ()) {
            reportError ("Attach varnode " + sym->getName () +
                             " is wrong size for list",
                         true);
        }
        int4 sz = 0;
        ...
        symtab.replaceSymbol (sym, new VarnodeListSymbol (sym->getName (),
                                                          patval, *varlist));
    }
    delete varlist;
    delete symlist;
...
PatternEquation* SleighCompile::constrainOperand (OperandSymbol* sym,
                                                  PatternExpression* patexp)

{ // Create constraint on operand
    PatternEquation* res;
    FamilySymbol* famsym =
        dynamic_cast<FamilySymbol*> (sym->getDefiningSymbol ());
    if (famsym !=
        (FamilySymbol*)0) { // Operand already defined as family symbol
...
void SleighCompile::defineOperand (OperandSymbol* sym,
                                   PatternExpression* patexp)
...

{
    ...
    try {
        if ((tp == SleighSymbol::value_symbol) ||
            (tp == SleighSymbol::context_symbol)) {
...
{ // Define operand as global symbol of same name
    TripleSymbol* glob =
        dynamic_cast<TripleSymbol*> (symtab.findSymbol (sym->getName (), 1));
...
ConstructTpl* SleighCompile::setResultStarVarnode (ConstructTpl* ct,
                                                   StarQuality* star,
                                                   VarnodeTpl* vn)
...
{ // Set constructors handle to be the value pointed
    // at by -vn-
    HandleTpl* res =
        new HandleTpl (star->id, ConstTpl (ConstTpl::real, star->size), vn,
...
bool SleighCompile::contextMod (vector<ContextChange*>* vec, ContextSymbol* sym,
...
{ // A temporary change to a context variable (within the parsing of a single instruction)
    ...
    for (uint4 i = 0; i < vallist.size (); ++i)
        if (dynamic_cast<const EndInstructionValue*> (vallist[i]) !=
            (const EndInstructionValue*)0)
            return false;
    ...
    ContextOp* op =
        new ContextOp (field->getStartBit (), field->getEndBit (), pe);
    vec->push_back (op);
    return true;
}
...
{ // A permanent (global) change to context.  During parsing of an instruction, this change
    ...
    ContextCommit* op = new ContextCommit (
        sym, field->getStartBit (), field->getEndBit (), cvar->getFlow ());
    vec->push_back (op);
}
...
{ // create a macro symbol (with parameter names)
    ...
    for (int4 i = 0; i < params->size (); ++i) {
        OperandSymbol* oper =
            new OperandSymbol ((*params)[i], i, (Constructor*)0);
        addSymbol (oper);
...
void SleighCompile::compareMacroParams (MacroSymbol* sym,
...
vector<OpTpl*>* SleighCompile::createMacroUse (MacroSymbol* sym,
...
{ // Create macro build directive, given symbol and parameters
    ...
    VarnodeTpl* idvn =
        new VarnodeTpl (ConstTpl (getConstantSpace ()),
...
SectionVector* SleighCompile::firstNamedSection (ConstructTpl* main,
                                                 SectionSymbol* sym)

{ // Start the first named p-code section after the main p-code section
    sym->incrementDefineCount ();
    SymbolScope* curscope =
        symtab.getCurrentScope (); // This should be a Constructor scope
    SymbolScope* parscope = curscope->getParent ();
    if (parscope != symtab.getGlobalScope ())
        throw LowlevelError (
            "firstNamedSection called when not in Constructor scope"); // Unrecoverable error
...
SectionVector* SleighCompile::nextNamedSection (SectionVector* vec,
...
{ // Add additional named p-code sections
    ...
    SymbolScope* curscope =
        symtab.getCurrentScope (); // This should now be the Constructor scope
    SymbolScope* parscope = curscope->getParent ();
    if (parscope != symtab.getGlobalScope ())
        throw LowlevelError (
            "nextNamedSection called when not in section scope"); // Unrecoverable
    symtab
        .addScope (); // Add new scope under the Constructor scope (not the last section scope)
    vec->append (section, curscope); // Associate finished section
    vec->setNextIndex (
        sym->getTemplateId ()); // Set index for the NEXT section (not been fully parsed yet)
    return vec;
}

SectionVector* SleighCompile::finalNamedSection (SectionVector* vec,
                                                 ConstructTpl* section)
...
{ // Create the crossbuild directive as a pcode template
    ...
    op->addInput (
        addr); // The first input is the VarnodeTpl representing the address
    op->addInput (
        sectionid); // The second input is the indexed representing the named pcode section to build
    res->push_back (op);
...
{ // Reset set state after a an error in previous constructor
    symtab.setCurrentScope (
        symtab.getGlobalScope ()); // Purge any dangling local scopes
}

bool SleighCompile::expandMacros (ConstructTpl* ctpl,
                                  const vector<ConstructTpl*>& macrotable)

{
    ...
    for (iter = ctpl->getOpvec ().begin (); iter != ctpl->getOpvec ().end ();
         ++iter) {
        op = *iter;
...
{ // Do all final checks, expansions, and linking for p-code sections
    ...
    for (;;) {
        ...
        errstring =
            checkSymbols (cur.scope); // Check labels in the section's scope
        if (errstring.size () == 0) {
            ...
            if (res == 2)
                errors.push_back (sectionstring +
                                  "Unnecessary BUILD statements");

            if (!PcodeCompile::propagateSize (cur.section))
                errors.push_back (sectionstring +
                                  "Could not resolve at least 1 variable size");
        }
        if (i < 0) { // These potential errors only apply to main section
            if (cur.section->getResult () !=
                (HandleTpl*)0) { // If there is an export statement
                if (big->getParent () == root)
                    errors.push_back (
                        "   Cannot have export statement in root constructor");
                else if (!force_exportsize (cur.section))
                    errors.push_back ("   Size of export is unknown");
            }
        }
        if (cur.section->delaySlot () !=
            0) { // Delay slot is present in this constructor
            ...
            if (cur.section->delaySlot () >
                maxdelayslotbytes) // Keep track of maximum delayslot parameter
...
{ // If the varnode is in the unique space, shift its offset up by -sa- bits
    if (vn->getSpace ().isUniqueSpace () &&
        (vn->getOffset ().getType () == ConstTpl::real)) {
...
{ // Shift the offset up by -sa- bits, for either the dynamic or static varnode aspects that are in the unique space
    if (hand->getSpace ().isUniqueSpace () &&
        (hand->getPtrSpace ().getType () == ConstTpl::real) &&
        (hand->getPtrOffset ().getType () == ConstTpl::real)) {
    ...
    if (hand->getTempSpace ().isUniqueSpace () &&
        (hand->getTempOffset ().getType () == ConstTpl::real)) {
...
{ // With crossbuilds,  temporaries may need to survive across instructions in a packet, so here we
    ...
    int4 sa = 8;
    int4 secsize =
        sections.size (); // This is the upper bound for section numbers
    SubtableSymbol* sym = root; // Start with the instruction table
    int4 i = -1;
    for (;;) {
        int4 numconst = sym->getNumConstructors ();
...
void SleighCompile::pushWith (SubtableSymbol* ss, PatternEquation* pateq,
                              vector<ContextChange*>* contvec)
...
void SleighCompile::buildConstructor (Constructor* big, PatternEquation* pateq,
                                      vector<ContextChange*>* contvec,
                                      SectionVector* vec)
...

{
    ...
    if (!expandMacros (rtl, macrotable)) {
        reportError ("Could not expand submacro in definition of macro " +
                         sym->getName (),
                     true);
        return;
    }
    PcodeCompile::propagateSize (
        rtl); // Propagate size information (as much as possible)
...
static int4 run_compilation (const char* filein, const char* fileout,
...

{
    ...
    try {
        ...
        cerr << "Unable to parse single input file as XML spec: " << filein
             << endl;
        exit (1);
    }
    s.close ();
            ...
            if (el->getName () == "processorfile") {
                ...
                for (int4 i = 0; i < num; ++i) {
                    if (el->getAttributeName (i) == "slaspec")
                        ...
                        compiler.setPreprocValue (el->getAttributeName (i),
    ...
    return run_compilation (specfilein.c_str (), specfileout.c_str (),
                            compiler);
}

static void findSlaSpecs (vector<string>& res, const string& dir,
                          const string& suffix)
...
static void initCompiler (SleighCompile& compiler, map<string, string>& defines,
                          bool enableUnnecessaryPcodeWarning,
                          bool disableLenientConflict,
                          bool enableAllCollisionWarning,
                          bool enableAllNopWarning, bool enableDeadTempWarning,
...

{
    ...
    if (argc < 2) {
        cerr << "USAGE: sleigh [-x] [-dNAME=VALUE] inputfile [outputfile]"
             << endl;
        cerr
        ...
        cerr << "   -n              print warnings for all NOP constructors"
             << endl;
        cerr << "   -t              print warnings for dead temporaries"
             << endl;
        cerr
    ...
    if (compileAll) {
        ...
        cout << "Compiling " << slaspecs.size () << " slaspec files in "
             << dirStr << "\n";
        for (int4 j = 0; j < slaspecs.size (); ++j) {
            string slaspec = slaspecs[j];
            cout << "Compiling (" << (j + 1) << " of " << slaspecs.size ()
                 << ") " << slaspec << "\n";
            string sla = slaspec;
            sla.replace (slaspec.length () - slaspecExtLen, slaspecExtLen,
                         SLAEXT);
            SleighCompile compiler;
            initCompiler (compiler, defines, enableUnnecessaryPcodeWarning,
                          disableLenientConflict, enableAllCollisionWarning,
                          enableAllNopWarning, enableDeadTempWarning,
                          enforceLocalKeyWord);
        ...
        if (i < argc - 1) {
            ...
            retval = run_compilation (fileinExamine.c_str (),
                                      fileoutExamine.c_str (), compiler);
        } else {
