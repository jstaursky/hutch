...
/// from the source block. NOTE: The control-flow direction of the edge can only
/// be determined from context, whether the edge is in the incoming or outgoing
/// edge list.
struct BlockEdge {
  ...
  // Restore \b this edge from an XML stream
...
public:
    ...
    ///
    /// The first four flags describe attributes of the blocks primary exiting
    /// edges The f_interior_* flags do not necessarily apply to these edges. They
    /// are used with the block structure and hierarchy algorithms where
    /// unstructured jumps are removed from the list of primary edges. These flags
    /// keep track only of the existence of unstructured edges, even though they
    /// aren't listed
    enum block_flags {
...
public:
    ...
    // Order \e case components of any contained BlockSwitch
    ...
    // Write out \b this to an XML stream
    void saveXml(ostream &s) const;
