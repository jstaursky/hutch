...
/// element and return iterator to this element. Pass back \b intersect value:
...

{
    ...
    if (iter != themap.begin ())
        --iter;
    if ((iter != themap.end ()) &&
        (-1 == addr.overlap (0, (*iter).first, (*iter).second.size)))
        ++iter;
    ...
    if ((iter != themap.end ()) &&
        (-1 !=
         (where = addr.overlap (0, (*iter).first, (*iter).second.size)))) {
        if (where + size <= (*iter).second.size) {
            // Completely contained in previous element
            intersect = ((*iter).second.pass < pass) ? 2 : 0;
            return iter;
        }
        addr = (*iter).first;
        size = where + size;
    ...
    while ((iter != themap.end ()) &&
           (-1 != (where = (*iter).first.overlap (0, addr, size)))) {
...
/// If the given address was heritaged, return (the iterator to) the SizeMap
/// entry describing the associated range and when it was heritaged.
