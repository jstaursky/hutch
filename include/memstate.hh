...
public:
    ...
    static void deconstructValue (uint1* ptr, uintb val, int4 size,
                                  bool bigendian);
};

/// A MemoryBank is instantiated with a \e natural word size. Requests for
/// arbitrary byte ranges may be broken down into units of this size. \return
/// the number of bytes in a \e word.
...
/// A MemoryBank is instantiated with a \e natural page size. Requests for large
/// chunks of data may be broken down into units of this size. \return the
/// number of bytes in a \e page.
...
/// A MemoryBank is a contiguous sequence of bytes associated with a particular
/// address space. \return the AddressSpace associated with this bank.
...
/// from the LoadImage. Other addresses in the space are filled in with zero.
...
/// write" behavior.
///
/// Pages are copied from the underlying object only when there is a write. The
/// underlying access routines are overridden to make optimal use of this page
/// implementation. The underlying memory bank can be a \b null pointer in which
/// case, this memory bank behaves as if it were initially filled with zeros.
...
///
/// The initial state of the bank is taken from an \e underlying memory bank or
/// is all zero, if this bank is initialized with a \b null pointer. This
...
public:
    ...
    MemoryHashOverlay (AddrSpace* spc, int4 ws, int4 ps, int4 hashsize,
                       MemoryBank* ul);
};
...
/// The MemoryState needs a Translate object in order to be able to convert
/// register names into varnodes
