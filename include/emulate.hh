...
/// emulator. Breakpoints are either associated with a particular user-defined
/// pcode op, or with a specific machine address (as in a standard debugger).
/// Through the BreakTable object, an emulator can invoke breakpoints through
/// the two methods
///  - doPcodeOpBreak()
///  - doAddressBreak()
///
...
public:
    ...
    /// This routine provides the context for all breakpoints in the table.
    ...
    /// breakpoint and it was designed to \e replace the action of the pcode op,
    /// then \b true is returned. \param curop is the instance of a pcode op to
    /// test for breakpoints \return \b true if the action of the pcode op is
    ...
    ///
    /// Within the table, the first breakpoint which is designed to work with at
    /// this address is invoked. If there was a breakpoint, and if it was
    /// designed to \e replace the action of the machine instruction, then \b
    /// true is returned. \param addr is address to test for breakpoints \return
    /// \b true if the machine instruction has been replaced by a breakpoint
    virtual bool doAddressBreak (const Address& addr) = 0;
};

/// \brief A breakpoint object
///
/// This is a base class for breakpoint objects in an emulator. The breakpoints
/// are implemented as callback method, which is overridden for the particular
/// behavior needed by the emulator. Each derived class must override either
///   - pcodeCallback()
///   - addressCallback()
///
/// depending on whether the breakpoint is tailored for a particular pcode op or
/// for a machine address.
class BreakCallBack {
protected:
...
/// method must be called before the breakpoint can be invoked
...
/// This routine is invoked during emulation, if this breakpoint has somehow
/// been associated with this kind of pcode op. The callback can perform any
/// operation on the emulator context it wants. It then returns \b true if these
/// actions are intended to replace the action of the pcode op itself. Or it
/// returns \b false if the pcode op should still have its normal effect on the
/// emulator context. \param op is the particular pcode operation where the
/// break occurs. \return \b true if the normal pcode op action should not occur
...
/// This routine is invoked during emulation, if this breakpoint has somehow
/// been associated with this address. The callback can perform any operation on
/// the emulator context it wants. It then returns \b true if these actions are
/// intended to replace the action of the \b entire machine instruction at this
/// address. Or it returns \b false if the machine instruction should still be
/// executed normally.
...
/// individual pcode operations as its atomic operation. The interface allows
/// execution stepping through individual pcode operations. The interface allows
/// querying of the \e current pcode op, the current machine address, and the
...
protected:
    ...
    ///
    /// This routine performs a standard p-code BRANCH operation on the memory
    /// state. This same routine is used for CBRANCH operations if the condition
    ...
    /// It does \e not perform the actual branch. \return the boolean state
...
/// to terminate and restart the main emulator loop as needed. The emulator
/// itself makes no use of this routine or the associated state variable \b
/// emu_halted.
...
/// Applications and breakpoints can use this method and its companion setHalt()
/// to terminate and restart the main emulator loop as needed. The emulator
/// itself makes no use of this routine or the associated state variable \b
/// emu_halted. \return \b true if the emulator is in a "halted" state.
...
/// \brief An abstract Emulate class using a MemoryState object as the backing
/// machine state
///
...
/// setExecuteAddress() is called to indicate which instruction is being
/// executed. The derived class must provide
///   - fallthruOp()
///   - setExecuteAddress()
///   - getExecuteAddress()
///
/// The following p-code operations are stubbed out and will throw an exception:
/// CALLOTHER, MULTIEQUAL, INDIRECT, CPOOLREF, SEGMENTOP, and NEW. Of course the
/// derived class can override these.
...
/// instructions into pcode and caches pcode ops for later use by the emulator.
/// The pcode is cached as soon as the execution address is set, either
...
/// machine state may be halted in the \e middle of a single machine
/// instruction, unlike conventional debuggers. This routine can be used to
/// determine if execution is actually at the beginning of a machine
/// instruction.
...
/// This routine can be used to determine where, within the sequence of ops in
/// the translation of the entire machine instruction, the currently executing
/// op is.
...
/// This routine can be used to examine ops other than the currently executing
/// op in the machine instruction's translation sequence.
...
/** \page sleighAPIemulate The SLEIGH Emulator
  ...
  \b SLEIGH provides a framework for emulating the processors which have a
   specification written for them. The key classes in this framework are:
  ...
  The MemoryState object holds the representation of registers and memory during
  emulation. It understands the address spaces defined in the \b SLEIGH
  specification and how data is encoded in these spaces. It also knows any
  register names defined by the specification, so these can be used to set or
  query the state of these registers naturally.

  The emulation framework can be tailored to a particular environment by
  creating \b breakpoint objects, which derive off the BreakCallBack interface.
  These can be used to create callbacks during emulation that have full access
  to the memory state and the emulator, so any action can be accomplished. The
  breakpoint callbacks can be designed to either augment or replace the
  instruction at a particular address, or the callback can be used to implement
  the action of a user-defined pcode op. The BreakCallBack objects are managed
  by the BreakTable object, which takes care of invoking the callback at the
  appropriate time.

  The Emulate object serves as a basic execution engine. Its main method is
  Emulate::executeCurrentOp() which executes a single pcode operation on the
  memory state. Methods exist for querying and setting the current execution
  address and examining the pcode op being executed.

  The main implementation of the Emulate interface is the EmulatePcodeCache
  object. It uses SLEIGH to translate machine instructions as they are executed.
  The currently executing instruction is translated into a cached sequence of
  pcode operations. Additional methods allow this entire sequence to be
  inspected, and there is another stepping function which allows the emulator to
  be stepped through an entire machine instruction at a time. The single pcode
  stepping methods are of course still available and the two methods can be used
  together without conflict.

  \section emu_membuild Building a Memory State

  Assuming the SLEIGH Translate object and the LoadImage object have already
  been built (see \ref sleighAPIbasic), the only required step left before
  instantiating an emulator is to create a MemoryState object. The MemoryState
  object can be instantiated simply by passing the constructor the Translate
  object, but before it will work properly, you need to register individual
  MemoryBank objects with it, for each address space that might get used by the
  emulator.

  A MemoryBank is a representation of data stored in a single address space
  There are some choices for the type of MemoryBank associated with an address
  space. A MemoryImage is a read-only memory bank that gets its data from a
  LoadImage. In order to make this writeable, or to create a writeable memory
  bank which starts with its bytes initialized to zero, you can use a
  MemoryHashOverlay or a MemoryPageOverlay.

  A MemoryHashOverlay overlays some other memory bank, such as a MemoryImage. If
  you read from a location that hasn't been written to directly before, you get
  the data in the underlying memory bank. But if you write to this overlay, the
  value is stored in a hash table, and subsequent reads will return this value.
  Internally, the hashtable stores values in a \e preferred wordsize only on
  aligned addresses, but this is irrelevant to the interface. Unaligned requests
  are split up and handled transparently.

  A MemoryPageOverlay overlays another memory bank as well. But it implements
  writes to the bank by caching memory \e pages. Any write creates an aligned
  page to hold the new data. The class takes care of loading and filling in
  pages as needed.

  Here is an example of instantiating a MemoryState and registering memory banks
  for a \e ram space which is initialized with the load image. The \e ram space
  is implemented with the MemoryPageOverlay, and the \e register space and the
  \e temporary space are implemented using the MemoryHashOverlay.
  ...
  \code
    void setupMemoryState(Translate &trans,LoadImage &loader) {
      ...
      MemoryHashOverlay registerstate(trans.getSpaceByName("register"),
                                                            8, 4096, 4096,
                                                            (MemoryBank *)0);
      MemoryHashOverlay tmpstate(trans.getUniqueSpace(),8,4096,4096,
                                    (MemoryBank *)0);
  ...
  All the memory bank constructors need a preferred wordsize, which is most
  relevant to the hashtable implementation, and a page size, which is most
  relevant to the page implementation. The hash overlays need an additional
  initializer specifying how big the hashtable should be. The null pointers
  passed in, in place of a real memory bank, indicate that the memory bank is
  initialized with all zeroes. Once the memory banks are instantiated, they are
  registered with the memory state via the MemoryState::setMemoryBank() method.

  \section emu_breakpoints Breakpoints

  In order to provide behavior within the emulator beyond just what the core
  instruction emulation provides, the framework supports \b breakpoint classes.
  A breakpoint is created by deriving a class from the BreakCallBack class and
  overriding either BreakCallBack::addressCallback() or
  BreakCallBack::pcodeCallback(). Here is an example of a breakpoint that
  implements a standard C library \e puts call an the x86 architecture. When the
  breakpoint is invoked, a call to \e puts has just been made, so the stack
  pointer is pointing to the return address and the next 4 bytes on the stack
  are a pointer to the string being passed in.
  ...
  Notice that the callback retrieves the value of the stack pointer by name.
  Using this value, the string pointer is retrieved, then the data for the
  actual string is retrieved. After dumping the string to standard out, the
  return address is recovered and the \e return instruction is emulated by
  explicitly setting the next execution address to be the return value.

  \section emu_finalsetup Running the Emulator
  Here is an example of instantiating an EmulatePcodeCache object. A breakpoint
  is also instantiated and registered with the BreakTable.
  ...
  Notice how the initial stack pointer and initial execute address is set up.
  The breakpoint is registered with the BreakTable, giving it a specific
  address. The executeInstruction method is called inside the loop, to actually
  run the emulator. Notice that a disassembly of each instruction is printed
  after each step of the emulator.

  Other information can be examined from within this execution loop or in other
  tailored breakpoints. In particular, the Emulate::getCurrentOp() method can be
  used to retrieve the an instance of the currently executing pcode operation.
  From this starting point, you can examine the low-level objects:
    - PcodeOpRaw   and
    - VarnodeData
 */
