...
public:
    ...
    SeqNum(const Address &a, uintm b) : pc(a)
    {
        uniq = b;
    }

    /// Copy a sequence number
    SeqNum(const SeqNum &op2) : pc(op2.pc)
    {
        uniq = op2.uniq;
    }
...
public:
    ...
    {
        return spc;    ///< Get the address space containing \b this Range
    }
    uintb getFirst(void) const
    {
        return first;    ///< Get the offset of the first byte in \b this Range
    }
    uintb getLast(void) const
    {
        return last;    ///< Get the offset of the last byte in \b this Range
    }
    Address getFirstAddr(void) const
    {
        return Address(spc, first);   ///< Get the address of the first byte
    }
    Address getLastAddr(void) const
    {
        return Address(spc, last);   ///< Get the address of the last byte
    }
...
public:
    ...
    {
        tree = op2.tree;    ///< Copy constructor
    }
    RangeList(void) {}						///< Construct an empty container
    void clear(void)
    {
        tree.clear();    ///< Clear \b this container to empty
    }
    bool empty(void) const
    {
        return tree.empty();    ///< Return \b true if \b this is empty
    }
    set<Range>::const_iterator begin(void) const
    {
        return tree.begin();    ///< Get iterator to beginning Range
    }
    set<Range>::const_iterator end(void) const
    {
        return tree.end();    ///< Get iterator to ending Range
    }
    int4 numRanges(void) const
    {
        return tree.size();    ///< Return the number of Range objects in container
    }
