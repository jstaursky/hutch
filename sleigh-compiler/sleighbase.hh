...
public:
    ...
    {
        return symtab.findSymbol(nm);    ///< Find a specific SLEIGH symbol by name in the current scope
    }
    SleighSymbol *findSymbol(uintm id) const
    {
        return symtab.findSymbol(id);    ///< Find a specific SLEIGH symbol by id
    }
    SleighSymbol *findGlobalSymbol(const string &nm) const
    {
        return symtab.findGlobalSymbol(nm);    ///< Find a specific global SLEIGH symbol by name
    }
